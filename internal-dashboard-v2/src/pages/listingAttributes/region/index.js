...
function Region() {
  ...
  const handleEditOpen = (item) => {
    const { id, name, slug, icon_path, sequence_no , parent_id } = item;
    // console.log(sequence_no);
    setEditData({ id, name : name?.props?.children[1]?.props?.children, slug, icon_path, sequence_no,parent_id : parent_id && parent_id });
  ...
  const handlePutRequest = (formData) => {
    ...
    putRegion(formData, formData.id)
      .then(() => {
  ...
  return (
    <div className="m-3">
      <PageHeader title="Regions" onClick={handleCreateOpen} />
      {rows && (
        <Table
          rows={rows}
      ...
      {pop.isEdit && (
        ...
        >
          <RegionForm
            ...
            editData={{ ...editData, parent_type: "country" }}
