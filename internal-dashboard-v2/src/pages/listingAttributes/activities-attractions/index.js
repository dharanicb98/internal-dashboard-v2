...
import ActivitiesAndAttractionsForm from "./activitiesAndAttractionsForm";
import { createActivities, deleteActivities, getAllActivities, updateActivities } from "../../../services/activities-attraction";
...
function ActivitiesAndAttractions() {
  ...
  const handleEditOpen = (item) => {
    const { id, name, slug, icon_path, sequence_no } = item;
    // console.log(sequence_no);
    setEditData({ id, name : name?.props?.children[1]?.props?.children, slug, icon_path, sequence_no });
  ...
  const handlePutRequest = (formData) => {
    ...
    updateActivities(formData, formData.id)
      .then(() => {
  ...
  const onDeleteList = () => {
    ...
    dispatch(loaderStart());
    deleteActivities(deletedObj.id)
      .then(() => {
        dispatch(loaderSuccess());
        setPageLoad((prev) => !prev);
        // console.log("success");
  ...
  return (
    <div className="m-3">
      <PageHeader title="Activities & Attractions" onClick={handleCreateOpen} />
      {rows && (
        <Table
          rows={rows}
      ...
      {pop.isCreate && (
        ...
        >
          <ActivitiesAndAttractionsForm
            ...
            editData={{ name: "",type: "", icon_path: "", sequence_no: "", description : "" }}
...
export default ActivitiesAndAttractions;
