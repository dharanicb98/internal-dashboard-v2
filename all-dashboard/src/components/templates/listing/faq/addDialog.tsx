...
export default function AddDialog(props: AddAddonsDialogProps) {
  ...
  React.useEffect(() => {
    const selectedService = listingDetails?.faq[index] || {};
    if (open == "edit") {
              ...
              return <div className="mt-5 border-t-2 pt-3" key={v._id + '-' + formData.details.length}>
                ...
                <div className="mt-3">
                  <div className="w-[100%]">
                    ...
                    <FilledInput
                      title="question"
                      value={formData.details[i].question}
                      onChange={(e) => setNewFormData('question', i, e.target.value)}
                    />
                  </div>
                  <div className="w-[100%] mt-2">
                    ...
                    <FilledInput
                      title="answer"
                      value={formData.details[i].answer}
                      onChange={(e) => setNewFormData('answer', i, e.target.value)}
                    />
                  </div>
                </div>
              </div>
