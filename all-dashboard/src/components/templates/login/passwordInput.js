...
function PasswordInput({onChange, value, type="text", label, labelClass="", inputClass="", className="", showImage=false, showPassword=false, handleShowPassword=() => {}}) {
  let id = useId()


  return (
    <div className={`relative ${className}  rounded-lg  md:w-full`}>
      <input
        id={`${id}-${label}`}
        value={value}
        onChange={onChange}
    ...
    peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1 ${labelClass}`}
      ...
      {showImage && (
        <div
          className="cursor-pointer absolute top-0 right-2 transform -translate-y-1/2"
...
const EyeOpen = () => {
  return (
     <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 absolute top-5 right-4 text-gray-500">
