...
export interface ItemProps<Element extends HTMLElement = HTMLDivElement> {
...
interface DropdownMenuResponse<
  ButtonElement extends HTMLElement,
  ItemElement extends HTMLElement,
> {
  readonly buttonProps: ButtonProps<ButtonElement>;
  readonly itemProps: ItemProps<ItemElement>[];
...
>(
...
): DropdownMenuResponse<ButtonElement, ItemElement> {
  // Use state
  ...
  const itemRefs = useMemo<React.RefObject<HTMLDivElement>[]>(
    () => Array.from({ length: itemCount }, () => createRef<HTMLDivElement>()),
    [itemCount]
  );
  ...
  const itemListener = (e: React.KeyboardEvent<HTMLDivElement>): void => {
  ...
  const itemProps: any = Array.from(
    { length: itemCount },
    (_ignore, index) => ({
